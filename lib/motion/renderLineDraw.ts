...
export function renderLine(spec: VisualSpec, theme: VisualTheme, g: Geom): string {
  ...
  const seriesSvg = bundle.series
    .map((item, seriesIndex) => {
      ...
      const area =
        !bundle.multi && coordinates.length
          ? path(
              {
                d: `${lineD} L ${coordinates[coordinates.length - 1].x.toFixed(2)} ${(plot.y + plot.height).toFixed(2)} L ${coordinates[0].x.toFixed(2)} ${(plot.y + plot.height).toFixed(2)} Z`,
                fill: item.color,
                opacity: 0.14
              },
              animate("opacity", 0, 0.14, 520, delay + 140, { easing: "ease-out" })
            )
          : "";
      ...
      const linePath = path(
        {
          ...
          fill: "none",
          stroke: item.color,
          "stroke-width": bundle.multi ? 2.4 : 3,
      ...
      const dots = coordinates
        .filter((_, index) => coordinates.length <= 18 || index === coordinates.length - 1)
        .map((coord, index, visible) => {
          const dotDelay = delay + Math.round((spec.motion.durationMs * index) / Math.max(visible.length - 1, 1)) + 90;
          return circle(
            {
              cx: round(coord.x),
              cy: round(coord.y),
              r: bundle.multi ? 3 : 3.4,
              fill: theme.surface,
              stroke: item.color,
              "stroke-width": 2,
              opacity: 1
      ...
      return group(area + linePath + dots);
    })
    .join("");
