import { animate, group, line, rect, textNode } from "@/lib/motion/svgPrimitives";
...
export function renderHorizontalBar(spec: VisualSpec, theme: VisualTheme, g: Geom): string {
  ...
  const baseline = line({ x1: round(barX), x2: round(barX + barWidth), y1: round(top + step * count), y2: round(top + step * count), stroke: theme.axis, "stroke-width": 1 });
  ...
  return points
    .map((point, index) => {
      ...
      const isEmpty = max <= 0 || point.value <= 0;
      const targetWidth = isEmpty ? 0 : Math.max(3, (Math.max(0, point.value) / max) * barWidth);
          ...
          (step >= 13
            ? textNode(
                formatNumber(point.value),
                {
                  ...
                  fill: isEmpty ? theme.soft : theme.strong,
                  opacity: 1,
                  "font-size": valueSize,
                  "font-family": FONT,
                  "font-weight": 700
                },
                animate("opacity", 0, 1, 300, isEmpty ? 0 : delay + spec.motion.durationMs - 130, { easing: "ease-out" })
              )
            : "")
      );
    })
