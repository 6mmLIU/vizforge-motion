...
- **可视化模板系统**：内置柱状图、横向排行、堆叠柱状、增长趋势折线、双系列面积趋势、渠道占比圆环、饼图、玫瑰构成图、矩形树图、活动热力图、散点/气泡图和单指标卡。
...
精简为一组始终忠实展示真实数据的图表，按数据故事分为对比、趋势、构成、分布四类：

| 类型 | slug | 适用场景 |
| --- | --- | --- |
| 柱状图 | bar | 销售表现、增长数据、月度指标 |
| 横向排行 | horizontal-bar | 渠道排行、产品排行、榜单对比 |
| 堆叠柱状 | stacked-bar | 多系列构成对比（由 series 字段驱动） |
| 增长趋势折线 | line | 时间趋势、访问量、收入变化（支持多系列） |
| 双系列面积趋势 | area | 双系列趋势对比，例如自然流量 vs 付费广告 |
| 活动热力图 | heatmap | 全年每日活跃度、签到、提交频次、发布节奏 |
| 渠道占比圆环 | donut | 构成占比、渠道来源、预算分布 |
| 饼图 | pie | 份额构成 |
| 玫瑰构成图 | rose | 具有视觉传播感的构成关系 |
| 矩形树图 | treemap | 层级占比、构成对比 |
| 散点 / 气泡图 | scatter / bubble | 两到三维指标相关性 |
| 单指标卡 | metric-card | 单指标卡、核心 KPI、迷你趋势 |

> 旧版的 `grouped-bar`、`arc`、`gauge`、`ranking`、`bar-race`、`slope`、`bump`、`timeline`、`line-race`、`waterfall`、`radar`、`sankey`、`network` 已被精简；`/api/v1/render` 会把这些旧 slug 自动映射到最接近的保留类型，老调用不会报错。

主题皮肤共 4 套，全部复用同一套排版引擎，仅切换配色：`light`（默认）、`dark`、`warm`、`mono`，保证任意主题、任意尺寸下预览与导出一致。
